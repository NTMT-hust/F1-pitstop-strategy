...
def map_telemetry_to_laps(
    ...
    telemetry_df = convert_time_columns(
        telemetry_df
    )

    telemetry_df['DriverNumber'] = (
        telemetry_df['Driver']
        .astype(str)
    )

    laps_df['DriverNumber'] = (
        laps_df['DriverNumber']
        .astype(str)
    )
    ...
    for driver in laps_df['DriverNumber'].unique():
        ...
        for i in range(len(driver_laps)):
            ...
            start_time = current_lap[
                'LapStartTime_Seconds'
            ]

            if i < len(driver_laps) - 1:

                end_time = driver_laps.iloc[i + 1][
                    'LapStartTime_Seconds'
                ]
        ...
        for lap_num, start, end in lap_intervals:
            ...
            mask = (
                ...
                &
                (
                    telemetry_df['SessionTime_Seconds']
                    >= start
                )
                &
                (
...
def aggregate_telemetry(
    ...
    telemetry_df = map_telemetry_to_laps(
        laps_df,
        telemetry_df
    )

    telemetry_features = telemetry_df.groupby(
    ...
    telemetry_features.columns = [
        "_".join(col)
        for col in telemetry_features.columns
    ]
...
def merge_telemetry_features(
    ...
    telemetry_features['DriverNumber'] = (
        telemetry_features['DriverNumber']
        .astype(str)
    )
...
def add_core_features(df):
    ...
    df['race_progress_fraction'] = (
        df['LapNumber'] / max_laps
    )

    df['expected_tyre_life'] = (
        df['Compound']
        .map(EXPECTED_TYRE_LIFE)
    )

    df['relative_tire_age'] = (
        df['TyreLife'] /
        df['expected_tyre_life']
    )

    df['tire_compound_age_delta'] = (
        df['TyreLife'] -
        df['expected_tyre_life']
    )
...
def add_telemetry_features(df):
    ...
    df['thermal_stress_proxy'] = (
        df['Throttle_mean'] *
        df['Brake_sum'] *
        df['Speed_std']
    )

    df['high_speed_stress'] = (
        df['Speed_mean'] *
        df['Throttle_mean']
    )
...
def preprocess_race_weekend(
    ...
    laps_df = clean_laps_dataframe(
        laps_df
    )

    laps_df = convert_time_columns(
        laps_df
    )

    df = merge_telemetry_features(
        laps_df,
        telemetry_df
    )
    ...
    df = add_historical_pit_features(
        df
    )
...
def process_all_races(
    ...
    for year in os.listdir(laps_root):
        ...
        for file in os.listdir(year_path):
            ...
            race_name = file.replace(
                "_Laps.csv",
                ""
            )

            laps_csv = os.path.join(
                year_path,
                file
            )
            ...
            if not os.path.exists(
                ...
                print(
                    f"Telemetry missing: {race_name} {telemetry_csv}"
                )

                continue

            print(
                f"Processing {year} {race_name}"
            )

            try:

                processed_df = (
                    preprocess_race_weekend(
                        laps_csv,
                        telemetry_csv
                    )
                )
...
if __name__ == "__main__":
    ...
    LAPS_ROOT = (
        r"C:\Nguyen Tri\Code\Statisanalyss\Data\f1_data_csv_export"
    )
